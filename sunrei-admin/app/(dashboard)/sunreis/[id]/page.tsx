...
import { SunreiDTO, UpdateSunreiRequest } from '@/api';
...
export default function EditSunreiPage() {
  ...
  const [sunrei, setSunrei] = useState<SunreiDTO | null>(null);
  ...
  const fetchSunrei = async () => {
    try {
      const response = await adminApi.adminGetSunrei(params.id as string);
      ...
      console.error('Failed to fetch sunrei:', err);
      setError('Failed to load Sunrei');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (data: UpdateSunreiRequest) => {
    try {
      setError(null);
      await adminApi.updateSunrei(params.id as string, data);
      router.push('/sunreis');
    } catch (err: any) {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Edit Sunrei</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-4xl">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {spotFields.map((field, index) => (
            <div key={field.id} className="border rounded-lg p-4 mb-4">
              ...
              <input
                type="hidden"
                {...register(`spots.${index}.id` as const)}
              />
        ...
        <div className="flex justify-end space-x-3">
          ...
          >
            {isSubmitting ? 'Updating...' : 'Update Sunrei'}
          </button>
        </div>
      </form>
    </div>
...
function PlacesInput({ control, spotIndex, register }: any) {
  ...
  return (
    <div>
      ...
      {fields.map((field: any, placeIndex: number) => (
        <div key={field.id} className="bg-gray-50 rounded p-3 mb-2">
          ...
          <input
            type="hidden"
            {...register(`spots.${spotIndex}.places.${placeIndex}.id` as const)}
          />
